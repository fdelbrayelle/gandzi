...
const lines = ref<BudgetLine[]>([
  { id: 'income', label: 'Income', kind: 'income', values: [4400, 4400, 4400, 4400, 4400, 4400, 4400, 4400, 4400, 4400, 4400, 4400] },
  { id: 'mortgage', label: 'Mortgage', kind: 'expense', values: [1400, 1400, 1400, 1400, 1400, 1400, 1400, 1400, 1400, 1400, 1400, 1400] },
  { id: 'groceries', label: 'Groceries', kind: 'expense', values: [500, 485, 470, 490, 510, 500, 520, 505, 495, 500, 515, 510] },
  { id: 'transport', label: 'Transport', kind: 'expense', values: [90, 80, 75, 88, 95, 92, 96, 94, 87, 90, 91, 93] },
  { id: 'utilities', label: 'Electricity / Gas', kind: 'expense', values: [194, 194, 194, 194, 194, 194, 194, 194, 194, 194, 194, 194] },
  { id: 'water', label: 'Water', kind: 'expense', values: [35, 35, 35, 35, 35, 35, 35, 35, 35, 35, 35, 35] },
  { id: 'insurance', label: 'Insurance', kind: 'expense', values: [174, 174, 174, 174, 174, 174, 174, 174, 174, 174, 174, 174] },
  { id: 'subscriptions', label: 'Subscriptions', kind: 'expense', values: [130, 130, 130, 130, 130, 130, 130, 130, 130, 130, 130, 130] },
  { id: 'vacation', label: 'Vacation', kind: 'expense', values: [0, 0, 0, 300, 0, 0, 1100, 0, 0, 0, 0, 300] },
]);
...
function onValueInput(line: BudgetLine, monthIdx: number, raw: string): void {
  const parsed = Number(raw);
  line.values[monthIdx] = Number.isFinite(parsed) ? parsed : 0;
}
</script>
  ...
  <div class="budget-matrix-card">
    <div class="matrix-scroll">
      <table class="budget-matrix">
        <thead>
          <tr>
            <th class="sticky-col left-header">Year 2026</th>
        ...
        <tbody>
          <tr v-for="line in lines" :key="line.id" :class="line.kind === 'income' ? 'income-row' : 'expense-row'">
            <th class="sticky-col row-label">{{ line.label }}</th>
            <td v-for="(value, monthIdx) in line.values" :key="`${line.id}-${monthIdx}`">
              <input
                ...
                step="0.01"
                :value="value"
                @input="onValueInput(line, monthIdx, ($event.target as HTMLInputElement).value)"
              />
            </td>
          ...
          <tr class="net-row">
            <th class="sticky-col row-label">Free Cashflow</th>
